...
* Added: A vertical align option (Baseline/Top/Middle/Bottom) for those same icons, for when they sit too high or low next to the artist name after changing the icon size.
* Added: Two new blocks, Artist Name Link and Breadcrumb Icon Link (Biography/Events), for building your own breadcrumb layout inside an ordinary Group block instead of the all-in-one Artist Breadcrumb block — sizing/color use the block editor's normal Font Size/Text Color controls, and vertical alignment uses the Group block's own built-in control. The original Artist Breadcrumb block still works exactly as before if you already have it in your header (requires agnosis-theme 0.5.10+ for the new bundled header layout).
* Fixed: The new Group-based breadcrumb showed an empty box on the main site instead of nothing at all. It's now fully hidden off an artist subdomain, same as before.
