...
* Fixed: The "Mediums" checklist on the Artworks Quick Edit panel (and the artwork edit screen) no longer mixes every language's translation of every medium into one list — it now shows only the mediums for that artwork's own language. Previously all of them appeared together (e.g. seven different-language versions of "Watercolor" at once), which also made it possible to accidentally assign a wrong-language medium to an artwork.
