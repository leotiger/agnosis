...
- **AI opt-out lanes** — `photo@` skips AI enhancement and the quality-rejection gate for photographers whose image is the artwork itself, but still runs AI description/tagging; `pure@` goes further — content is never touched or rewritten by AI, title/body/alt text are published verbatim from the artist's own words, and a poster is auto-generated from the text when there's no attached photo. AI is only used to classify the medium and a few tags, so the post stays findable
