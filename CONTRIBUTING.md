...
- **Translations** — the plugin is fully translatable (`Text Domain: agnosis`, `.pot`/`.po`/`.mo` under `languages/`). Contributing or improving a translation is welcome even without touching any code — see `composer make-pot` / `composer compile-pos` in [Development setup](#development-setup) if you're regenerating source strings, and `composer translate-missing` (Anthropic, OpenAI, or Gemini via `--provider=`; needs that provider's API key, set as an env var or entered when prompted) for an AI-assisted first pass instead of hand-translating in Loco Translate.
...
- `docs/agnosis-update-manifest.php` — the self-hosted update endpoint's source (deployed separately to `agnosis.art` by a maintainer) — gets the **same treatment on every version bump**: `$version` and `$download_url` updated, and a new `<h4>X.Y.Z</h4>` block prepended to `$changelog`, trimmed to the two most recent releases (same two-version rule as `readme.txt`, stated in the file's own comment). This file went eleven versions stale before an audit caught it (§4b, `AUDIT-1.0.0.md`) — `dev/bin/build-zip.sh` already automates `$sha256`, `$sha256_note` (added for the fourteenth audit's §5b), and, since 2026-07-22, `$last_updated` too (set to the date of the local build — the release process builds immediately before shipping, so this is accurate for the intended workflow; re-run `build-zip.sh` right before uploading if a real gap ever opens up) — see the manifest's own comment. `$version`/`$download_url`/`$changelog` are deliberately still hand-edited (its own docblock explains why), so they only stay current if this step is actually done as part of the bump, not treated as optional. A hand version-bump still needs to reset `$sha256`/`$sha256_note`/`$last_updated` to their "not built yet" defaults — the script only runs at build time, not bump time.
- `readme.txt` also carries an `== Screenshots ==` section (added 2026-07-22) — five numbered captions matched positionally to `screenshot-1.png` … `screenshot-5.png`. If the plugin's actual UI changes enough that a caption no longer matches its screenshot, update both together; don't let the text drift from what the image actually shows. An `== Upgrade Notice ==` section was added the same day — unlike Changelog, this is NOT filled in for every release: only add a `= X.Y.Z =` entry (300-char cap) when that specific version has something a user genuinely needs to know before updating (a behavior change, a one-time self-heal, a breaking change), and leave versions without one out entirely rather than padding it with routine fixes.
...
2. Translate the newly-blanked strings for each locale — `composer translate-missing` (needs `ANTHROPIC_API_KEY`; on-demand, not run automatically by this checklist — see its own description via `composer` with no args) is the current preferred path over hand-translating in Loco Translate or editing the `.po` directly: it fills both ordinary untranslated strings and the plural-form-category gaps Loco's own editor UI doesn't surface (`AUDIT-0.9.44.md` §5d) in one pass, with no download/upload round-trip against a live site. `composer fill-loco-gaps` remains available for the narrower plural-gap case when you already have translations in hand rather than wanting the AI to draft them.
...
3. `composer build-zip` (from `dev/`) — packages the plugin per `.distignore`, and handles `docs/agnosis-update-manifest.php`'s `$sha256` field itself (cleared at the start of the run, recomputed once the zip is built, so a failed run never leaves a stale digest behind), keeping the companion `$sha256_note` status line and `$last_updated` (today's date) in sync at that same success step too. Output lands in `../agnosis-deploy/agnosis-<version>.zip`.
4. Tag the release (`vX.Y.Z`) and create a GitHub Release, attaching the zip from step 3.
5. Confirm `$version`/`$download_url` in `docs/agnosis-update-manifest.php` match the release just created (`$sha256`/`$sha256_note`/`$last_updated` are already written by step 3), then deploy that file to `wp-content/mu-plugins/` on `agnosis.art` — it's what every installed site's own update check reads. This file going stale for eleven versions once, unnoticed, is exactly what made it a standing "update every bump" rule (§4b, `AUDIT-1.0.0.md`).
