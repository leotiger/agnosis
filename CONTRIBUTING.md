...
- `docs/agnosis-update-manifest.php` — the self-hosted update endpoint's source (deployed separately to `agnosis.art` by a maintainer) — gets the **same treatment on every version bump**: `$version`, `$download_url`, and `$last_updated` updated, and a new `<h4>X.Y.Z</h4>` block prepended to `$changelog`, trimmed to the two most recent releases (same two-version rule as `readme.txt`, stated in the file's own comment). This file went eleven versions stale before an audit caught it (§4b, `AUDIT-1.0.0.md`) — `dev/bin/build-zip.sh` already automates the `$sha256` field (plus its companion `$sha256_note` status line, added for the fourteenth audit's §5b — see the manifest's own comment) at build time, but `$version`/`$download_url`/`$last_updated`/`$changelog` are deliberately still hand-edited (its own docblock explains why), so they only stay current if this step is actually done as part of the bump, not treated as optional. A hand version-bump still needs to reset `$sha256`/`$sha256_note` to their "not built yet" defaults — the script only runs at build time, not bump time.
...
3. `composer build-zip` (from `dev/`) — packages the plugin per `.distignore`, and handles `docs/agnosis-update-manifest.php`'s `$sha256` field itself (cleared at the start of the run, recomputed once the zip is built, so a failed run never leaves a stale digest behind), keeping the companion `$sha256_note` status line in sync at both of those steps too. Output lands in `../agnosis-deploy/agnosis-<version>.zip`.
