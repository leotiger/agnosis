...
class Settings {
	 ...
	 * site's own domain, and whether that domain publishes an SPF/DMARC/DKIM
	 * record at all (correctness/alignment is out of scope — a plugin can't
	 * know a shared host's outbound IP without an actual delivered test; DKIM
	 * is additionally best-effort only — see Deliverability::dkim_check()'s
	 * own docblock for why a "not found" there isn't proof of anything), plus
	 * whether the domain is listed on Spamhaus's DBL — a confirmed real-world
	 * cause of "sent successfully but never arrived", especially for a
	 * domain still building sending reputation. Also flags a detected
	 * SMTP-sending plugin, and offers the same one-click "send a test to my
	 * address" pattern the newsletter dashboard already uses
	 * (render_newsletter_test_form()) via CommunityMailer's own headers, so
	 * the test reflects the same identity workflow mail actually sends from.
	 */
